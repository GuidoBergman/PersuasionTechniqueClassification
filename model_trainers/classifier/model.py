...
def train_classifier(dataset: Dataset):
    ...
    def _tokenize_data(dataset: Dataset, tokenizer: RobertaTokenizerFast, label_all_tokens: bool = True) -> Dataset:
        ...
        tokenized_dataset = dataset.map(tokenize_and_align, batched=True, num_proc=os.cpu_count(), remove_columns=dataset["train"].column_names)
    ...
    def _compute_metrics(p):
        ...
        results = eval_metric.compute(predictions=str_pred, references=str_labels, zero_division=0)
        return {
